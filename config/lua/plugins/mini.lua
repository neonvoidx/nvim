...
do
	...
	map("v", "<leader>/", function()
		pick.grep({ pattern = visual_selection(), cwd = vim.uv.cwd() })
	end, { desc = "Grep selection" })
	map("n", "<leader>sg", function()
		pick.grep({ cwd = vim.uv.cwd() })
	end, { desc = "Grep" })
